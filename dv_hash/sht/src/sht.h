#ifndef SHIFTINGHASHTABLE17_SHT_H
#define SHIFTINGHASHTABLE17_SHT_H
...
//        }
    ...
    {
//        constexpr uint32_t max_hash = bf_k > d ? bf_k : d;
//        uint32_t hash_vals[max_hash];
//
//        for (uint32_t i = 0; i < max_hash; ++i) {
//            hash_vals[i] = hash_func[i](key);
//        }
//
        const uint32_t hash_vals[8] = {
            multiplicative_hash<0x7fb9b1ef>(key),
            multiplicative_hash<0xab35dd63>(key),
            multiplicative_hash<0x41ed960d>(key),
            multiplicative_hash<0xc7d0125f>(key),
            multiplicative_hash<0x071f9f8f>(key),
            multiplicative_hash<0x55ab55b9>(key),
            multiplicative_hash<0x479ab41d>(key),
            multiplicative_hash<0x9942f0a5>(key),
        };
...
//        uint32_t ret;
        ...
        if (bf_k) {
            for (int i = 0; i < bf_k; ++i) {
                uint32_t pos = hash_vals[i] % bf_size_in_bits;
                uint32_t bucket = pos >> 6;
                uint32_t offset = pos % 64;
                uint64_t bt = bf[bucket];
//
//            cout << ret << " " << __rorq(bt, offset) << endl;
                mask &= __rorq(bt, offset);
                if (!mask) {
                    break;
                }
            }
...
//            uint32_t pos = get_subtable_position(key, i);
            uint32_t pos = hash_vals[i] % bucket_per_subtable;
            Bucket & b = subtables[i][pos];

//            if (query_bucket(b, key, ret)) {
//                return ret;
//            }
            if (cell_per_bucket == 16) {
                __m256i simd_key = _mm256_set1_epi32(key);
                __m256i ra = _mm256_cmpeq_epi32(*(__m256i *)b.key, simd_key);
                __m256i rb = _mm256_cmpeq_epi32(*(__m256i *)(b.key + 8), simd_key);
                int mk = (_mm256_movemask_ps(*(__m256*)&rb) << 8) + _mm256_movemask_ps(*(__m256*)&ra);
//                __m128i simd_key = _mm_set1_epi32(key);
//                __m128i ra = _mm_cmpeq_epi32(*(__m128i *)b.key, simd_key);
//                __m128i rb = _mm_cmpeq_epi32(*(__m128i *)(b.key + 4), simd_key);
//                int mk = (_mm_movemask_ps(*(__m128*)&rb) << 4) + _mm_movemask_ps(*(__m128*)&ra);
                if (mk) {
                    int k = _tzcnt_u32(mk);
                    return b.val[k];
                }
                ...
                __m256i simd_key = _mm256_set1_epi32(key);
                __m256i ra = _mm256_cmpeq_epi32(*(__m256i *)b.key, simd_key);
                int mk = _mm256_movemask_ps(*(__m256*)&ra);
//                __m128i simd_key = _mm_set1_epi32(key);
//                __m128i ra = _mm_cmpeq_epi32(*(__m128i *)b.key, simd_key);
//                __m128i rb = _mm_cmpeq_epi32(*(__m128i *)(b.key + 4), simd_key);
//                int mk = (_mm_movemask_ps(*(__m128*)&rb) << 4) + _mm_movemask_ps(*(__m128*)&ra);
                ...
                false_positive_probe++;
            } else if (cell_per_bucket == 4) {
                __m128i simd_key = _mm_set1_epi32(key);
                __m128i ra = _mm_cmpeq_epi32(*(__m128i *)b.key, simd_key);
                int mk = _mm_movemask_ps(*(__m128*)&ra);
                if (mk) {
                    int k = _tzcnt_u32(mk);
                    return b.val[k];
                }
        ...
        uint32_t prefer_table = get_home_table(key);
        uint32_t pos = hash_vals[prefer_table] % bucket_per_subtable;
        Bucket & b = subtables[prefer_table][pos];
//        if (query_bucket(b, key, ret)) {
//            return ret;
//        }
        if (cell_per_bucket == 16) {
...
#endif //SHIFTINGHASHTABLE17_SHT_H
